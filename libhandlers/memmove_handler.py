...
import re
class memmove_handler(object):
    
    @staticmethod
    def gen_match_str(variable):
        access=variable.accessStr()
        if '|' not in access:
            return r"(?<![_A-Za-z0-9])memmove\s*\(\s*&\s*"+access+r"\s*,"
        else:
            pointerstr=variable.pointerStr()
            return r"(?<![_A-Za-z0-9])memmove\s*\(\s*"+pointerstr+r"\s*,"
    @staticmethod
    def isArgDef(variable,codestr):
        lib_definition=memmove_handler.gen_match_str(variable)
    ...
    def getJobs(index,varstr,codestr):
        syscall_definition=memmove_handler.gen_match_str(varstr)
        m=re.search(syscall_definition,codestr)
        start_pos=m.span()[1]
        end_pos,islast=ArgHandler.nextarg(codestr,start_pos)
        if end_pos is None:
            print "Error! memmove second arg wrong!" 
            x=1/0
        elif islast:
            print "Error! memmove third arg missing!" 
            x=1/0
        ...
        if end_pos is None or not islast :
            print "Error! memmove third arg wrong!" 
            x=1/0
